...
function fileNameChange(originalPath, name, id, fileType) {
  const uploadedFiles = []
  ...
  switch (fileType) {
    case 'video':
      newPath = path.join(__dirname, 'uploads/videos/', newName)
      break
    case 'homework':
      newPath = path.join(__dirname, 'uploads/homework/', newName)
      break
    case 'material':
      newPath = path.join(__dirname, 'uploads/material/', newName)
      break
    default:
